...
Each backed-up database file is AES-256-GCM encrypted at rest (`<name>.enc`, mode `0600`) with a recovery key from `CONTROL_PLANE_BACKUP_RECOVERY_KEY` (32-byte base64; `CONTROL_PLANE_BACKUP_RECOVERY_KEY_PREVIOUS` supports key rotation, same convention as `VAULT_ENCRYPTION_KEY`). This key is deliberately separate from the credential vault's own key, so a snapshot stays restorable even in a disaster scenario where the primary deployment — and its vault key — is gone. Development/test falls back to a key derived from `SESSION_SECRET`. Checksums recorded in the manifest are computed over the plaintext before encryption; verify and restore-preview both transparently decrypt (to memory, or to a `0600` temp file when SQLite needs a real file path) before checking, and report `recovery_key_unavailable` or `decryption_failed` per database if that fails.
...
- **Scoped automation access**: API tokens are hashed at rest, shown once at creation, can be permission-restricted, IP-restricted (an optional `allowedIps` list of exact IPv4/IPv6 addresses and/or IPv4 CIDR ranges; an empty list means unrestricted), and expired/revoked, and are accepted only by the versioned `/api/v1` surface. The surface's contract is published as an OpenAPI 3.0 document at `GET /api/v1/openapi.json` (no token required to read the document itself) and is kept in sync with the routes by `tests/unit/server/openapi-v1.test.js`. Tokens are issued, listed, and revoked from an "API Tokens" tab in the Governance Control Panel in the client (the raw secret is shown once, at creation, and cannot be retrieved again).
- **Compatibility policy**: the same document's `info['x-compatibility-policy']` publishes the actual rules for `/api/v1` — nothing already shipped there changes in a breaking way (no route/field removed or retyped, no auth-semantics change) without moving to a new `/api/v2` published alongside it; additive changes (new routes, new optional fields) can land in v1 without notice; and a route or field slated for removal is marked `deprecated: true` in the document and starts carrying `Deprecation`/`Sunset` (RFC 8594) response headers at least 90 days before removal, with the change recorded in `CHANGELOG.md`. Nothing in v1 is deprecated today — this is the standing policy for when something eventually is. `tests/unit/server/openapi-v1.test.js` asserts the policy block stays published.
