const TemplateCreateForm = {
  ...
  computed: {
    ...
    selectedSource() { return this.sources.find((source) => (source.profileId || source.ref) === this.draft.sourceRef) || null; },
    canSubmit() { return Boolean(this.draft.sourceRef && this.draft.nameLabel); },
  },
  watch: {
    sources: { immediate: true, handler(options) { if (!options.some((source) => (source.profileId || source.ref) === this.draft.sourceRef)) this.draft.sourceRef = options[0]?.profileId || options[0]?.ref || ''; } },
    kind() { this.draft.sourceRef = this.sources[0]?.profileId || this.sources[0]?.ref || ''; },
  },
  methods: {
    submit() { if (this.canSubmit) this.$emit('submit', { ...this.draft, sourceRef: this.selectedSource?.sourceRef || this.draft.sourceRef, operatingSystemProfileId: this.isOperatingSystem ? (this.selectedSource?.profileId || '') : '', kind: this.kind, tags: this.tagsInput.split(',').map((tag) => tag.trim()).filter(Boolean) }); },
  },
};

if (typeof module !== 'undefined') {
  module.exports = TemplateCreateForm;
}
